...
from .utils import * # Assuming this contains retrieve_username, create_needs_update_message, messages_content, button_content, etc.
...
class TelegramBot():
    ...
    def handle_donate(message):
        ...
        keyboard.add(types.InlineKeyboardButton(button_content['Crypto'], callback_data='donate_crypto'))
        # Using the defined constant for clarity
        keyboard.add(types.InlineKeyboardButton(button_content['TG_Stars'], callback_data=CALLBACK_DONATE_STARS_INITIAL))
        TelegramBot.bot.send_message(message.chat.id, donations_content['choose_donation_option_prompt'], reply_markup=keyboard)
    ...
    def handle_crypto_donation(call):
        ...
        coin_info = get_crypto_address_info(coin_key) # Assuming this function exists in .utils
    ...
    def default_message(message):
        ...
        if telegram_user_id in TelegramBot.admin_users and telegram_user_id in TelegramBot.admin_user_broadcasts:
            ...
            TelegramBot.bot.send_message(message.chat.id, messages_content['default_fallback'].format(support=os.getenv("SUPPORT_TG")), parse_mode='HTML')
